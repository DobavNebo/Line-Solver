...
func Tear(xas string) []Piece{
    ...
    for i, t := 0, len(x); i < t; {
        ...
        if ispon(c) {
            ...
            if warn > 1 { fmt.Println("Warning! Incorrect number: " + numStr + "\n It will be perceived as 0 (zero)") }
        ...
        result = append(result, Piece{string(c), 0, 0})
        i += 1
    }
	return result
}
